...
keysAltTT() { ;⎇K⃣  various symbols in a popup panel
  ...
  hk🛈("⇧⎇t​"  	,hkAltTT,,Map("h","Math"             	,"🔣",Ch["Math"       	].Join(delim:="") ,"🔖",Ch["Math🔖"],"🔖s",Ch["Math🔖"].Join(" "),"f",p.fname_(A_LineFile),"l№",A_LineNumber))
...
hkAltTT(hk_dirty) {
  ...
  Switch hk, 0 { ; Hotkey created → key name and ordering of its modifier symbols gets fixed
    ...
    case k("‹⎇``"	) : alt_tt_popup("Para"   ,'q')
    case k("⇧⎇1" 	) : alt_tt_popup("QuotesS",'q')
    case k("⇧⎇2" 	) : alt_tt_popup("QuotesD",'q')
    case k("⇧⎇4" 	) : R:=lRu(),csub(intersperse([],Ch["Currency"],,1) "`n" intersperse(Ch["CurrLab" R], Ch["Currency" R]),'M')
    case k("⇧⎇5" 	) : alt_tt_popup("Percent",'q')
    ...
    case k("⇧⎇b" 	) : alt_tt_popup("Bullet",'q')
