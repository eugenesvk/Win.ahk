/* use winAPIconst_t via a loader class
 ...
 */
      ; TODO: when alpha gets struct with strings

class winConstT {
  type_	: Str
  value	: Str
}
class winAPIconst_t_loader {
  static m := Map(
    'mmap' ,Map(
      'c',0,
      'libPath','lib\',
...
class winAPIconst_t { ; Various win32 API constants from a memory-mapped file
  __new(libPath,libNm) {
    this.libPath  	:= libPath
     ,this.libNm  	:= libNm
     ,this.ℯsz    	:= 100 ; max limit of error size to get
     ,this.hModule	:= 0
     ,this.lib𝑓   	:= this.libNm '\' 'get_win32_const_t' ; (pre:&WideChar,s:&WideChar, err_sz:u32,err_ptr:*mut WideChar) -> *mut WinConstFFI
  ...
  getKey(𝑓,key) {
    ...
    ,cFree	:= this.free
    if res_ptr := 𝑓(key,ℯ) {
      res_Struct := winConstT() ; insantiate a new object
      ; TODO: when alpha gets struct with strings
